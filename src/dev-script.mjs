...
const run = async () => {
  ...
  let input = web3.eth.abi.encodeParameters(
    ...
    ['0x15420b71', tokenContract.options.address, accounts[2], '500', '5', '0']);
  console.log(input);

  let toSignHexString = web3.utils.keccak256(input);
  console.log(toSignHexString);
  let toSignByteArray = web3.utils.hexToBytes(toSignHexString);

  let message = await web3.eth.accounts.sign(toSignByteArray, "3d63b5b61cc9636a143f4d2c56a9609eb459bc2f8f168e448b65f218893fef9f");
  console.log(toSignByteArray === message.message);
